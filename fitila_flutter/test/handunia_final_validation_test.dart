...
void main() {
  testWidgets('Handunia final map labels remain visible and readable', (
    tester,
  ) async {
  ...
  testWidgets('Handunia final trace confirms then returns automatically', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(390, 844);
    tester.view.devicePixelRatio = 1;
        ...
        home: Scaffold(
          body: Builder(
            builder: (context) => Column(
              children: [
                ElevatedButton(
                  key: const Key('open-trace-final'),
                  onPressed: () async {
    ...
    await tester.tap(find.byKey(const Key('open-trace-final')));
